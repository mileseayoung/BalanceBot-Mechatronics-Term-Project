...
interval = 25000 # Spitballing here
    ...
    while True:
        ...
        if utime.ticks_diff(currTime, nextTime) >= 0:
            ...
            if state == 1:
                ...
                X_dot_plat = Encoder1.getDelta() / (interval*1e6)
                plat_paramX = [X,X_dot_plat]
                
                #finding theta from 0 on Y axis
                Encoder2.update()
                ytick = Encoder2.getPosition()
                Y = Encoder2.tick2deg(ytick)
                Y_dot_plat = Encoder2.getDelta() / interval*(1e6)
                
                plat_paramY = [Y,Y_dot_plat]
                #print([Y,Y_dot])
                ...
                if current_ball_pos[0] == True:
                    ...
                    X_ball_dot = (current_ball_pos[1] - last_ball_pos[1]) / interval*(1e6)
                    Y_ball_dot = (current_ball_pos[2] - last_ball_pos[2]) / interval*(1e6)
                   
                    ball_paramX = [X_ball,X_ball_dot]
                    ball_paramY = [Y_ball,Y_ball_dot]
                    state = 2
                else:
                    print('ball left platform returning to calibration')
                    state = 0
            ...
            if state == 2:
                ...
                InputTx = (plat_paramX[0] * (-gains[3])) + ((-gains[1])*plat_paramX[1] + ((ball_paramX[0])*(-gains[2])) + ((ball_paramX[1])*(-gains[0])))
                InputTy = (plat_paramY[0] * (-gains[3])) + ((-gains[1])*plat_paramY[1] + ((ball_paramY[0])*(-gains[2])) + ((ball_paramY[1])*(-gains[0])))
